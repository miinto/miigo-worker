...
func main () {
	...
	for index < size {
		index++
		ch.Publish("", "go-generic-0-0", false, false, amqp.Publishing{Body: []byte(`{"_type":"Command\\ComplexCommand","_data":{"foo":"foocontent","bar":{"foo":"inner_foobar`+strconv.Itoa(index)+`"}}}`)})
	}
}